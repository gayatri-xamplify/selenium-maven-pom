...
public class ManageEventCampaignPage {
    ...
    private By manageEventCampaigns = By.xpath("//span[ contains(@class,'title')] [contains(text(),'Manage Campaigns')]");
    private By eventTab             = By.xpath("//li[contains(text(),'Event')]");

    // List view / grid view toggles
    private By hoverGridViewButton  = By.xpath("//div/app-manage-publish/div[1]/div[3]/div/div/div[1]/div/div[1]/div/div/div/div[1]/div/button");
    ...
    private By copySaveChanges      = By.xpath("//div[@id='saveAsModal']//button[@type='button'][normalize-space()='Save changes']");
    ...
    private By sendPreviewButton    = By.xpath("//div[@id='public-event-email-modal-popup']//button[contains(text(),'Send')]");
    private By sendPreviewClose     = By.xpath("//div[@id='public-event-email-modal-popup']//button[contains(text(),'Close')]");

    // Preview & Delete partners
    private By previewDeleteMenu    = By.xpath("//*[@id=\"manage-campaign-list\"]/tbody/tr[1]/td[4]/div/div/ul/li[3]/a");
    private By previewDeleteSortBy  = By.xpath("//div[1]/div[1]/app-preview-partners[1]/div[1]/div[2]/div[1]/div[1]/div[1]/div[2]/div[1]/select[1]");
    ...
    private By gridUpdateEventIcon        = By.xpath("//*[@id=\"manage-campaign-list\"]/tbody/tr[1]/td[4]/div/div/ul/section[2]/li/a/i");
    private By gridUpdateMessageTextarea  = By.xpath("//textarea[@placeholder='Add a comment...']");
    private By gridUpdateButton           = By.xpath("//label[@class='btn btn-primary button-border'][contains(text(),'Update')]");
    // Cancel event in grid (commented in original but kept as locators)
    private By gridCancelEventIcon        = By.xpath("//div[1]/div[3]/div/div/div[1]/div/div[3]/div/div/div[1]/div/div[5]/span[2]/a");
    ...
    // Analytics reports
    private By campaignBasedReportsIcon   = By.xpath("//*[@id=\"manage-campaign-list\"]/tbody/tr[2]/td[4]/div/a[3]/span/i");
    ...
    private By invitesReminderSend        = By.xpath("//*[@id=\"updateEventModal\"]//button[2]");
    ...
    private By yourPartnerRow             = By.xpath("//*[@id=\"partner-analytics\"]/div/div/div/table/tbody/tr/td[1]");
    private By yourPartnerInvitesTile     = By.xpath("//*[@id=\"values\"]/div[1]/button/div");
    private By yourPartnerCloseIcon       = By.xpath("//*[@id=\"timeline-view\"]/div/a");

    private By partnerRedisAnalyticsSearchBox = By.xpath("//div[1]/div[2]/div/div[2]/div/div/div/div[1]/div[2]/div/input");
    ...
    private By partnerRedisAnalyticsLeadsTile = By.xpath("//*[@id=\"float-right\"]/div[2]/button[@class='portlet light color1-invert crcb']");
...
//        WaitUtil.waitForPageToLoad(driver, 60);
    ...
    public void sendPreviewEmailAndHandlePreviewDelete() {
        ...
        /*
        safeClick(previewDeleteSingleDelete);
        safeClick(previewDeleteConfirm);
        */
...
//        sleep(800);
        ...
        // If you want, you may toggle views back to list again using same hoverGridViewButton etc.
    }

    // ===================== SECTION 6: Archive Event Campaigns from Grid =====================

    public void archiveAndDeleteEventFromGrid() {
        logger.info("Archiving and deleting Event campaign from grid view...");

        WaitUtil.waitForPageToLoad(driver, 60);

        // Directly navigate to manage campaigns page is responsibility of caller
        WaitUtil.waitAndClick(driver, eventTab, 60);
        WaitUtil.waitForPageToLoad(driver, 60);

//        // Hover on event card to reveal archive icon
//        WebElement card = WaitUtil.waitForElementVisible(driver, gridHoverEventCampaign, 60);
//        new Actions(driver).moveToElement(card).perform();
//        sleep(800);
//
//        safeClick(gridArchiveIcon);
//        WaitUtil.waitForPageToLoad(driver, 60);
//
//        safeClick(archivedCampaignsButton);
//        WaitUtil.waitForPageToLoad(driver, 60);
//        logger.info("Archived campaigns button clicked successfully.");
//
//        // Click grid view inside archived
//        safeClick(gridViewIcon);
//        WaitUtil.waitForPageToLoad(driver, 60);
//        logger.info("Clicked Grid View in archived campaigns.");

        WaitUtil.waitAndClick(driver, eventTab, 60);
        WaitUtil.waitForPageToLoad(driver, 60);

        // Hover & delete archived
        WebElement archivedCard = WaitUtil.waitForElementVisible(driver, gridDeleteArchivedIcon, 60);
        new Actions(driver).moveToElement(archivedCard).perform();
        sleep(800);

        safeClick(gridDeleteArchivedIcon);
        WaitUtil.waitForPageToLoad(driver, 60);
        safeClick(deleteArchivedYesButton);
        WaitUtil.waitForPageToLoad(driver, 60);
        logger.info("Deleted archived event campaign.");

        safeClick(activeCampaignsButton);
        WaitUtil.waitForPageToLoad(driver, 60);
        logger.info("Back to active campaigns page.");
    }

    // ===================== SECTION 7: Event Analytics & Reports =====================
    ...
    public void openEventAnalyticsAndReports() {
        ...
        // Your Partner tile
        safeClick(yourPartnerRow);
        WaitUtil.waitForPageToLoad(driver, 60);

        safeClick(yourPartnerInvitesTile);
        WaitUtil.waitForPageToLoad(driver, 60);
        ...
        if (leadsRedisTile.isEnabled()) {
            ...
            logger.info("Unable to click leads tile due to count is '0' in Partner Redistributed Campaign Analytics section.");
        }
