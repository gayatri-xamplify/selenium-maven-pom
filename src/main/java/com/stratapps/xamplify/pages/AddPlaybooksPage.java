...
public class AddPlaybooksPage {
	...
	public void enterPlaybookTitle(String title) {
	...
	public void selectAsset(String type) throws Exception {
		...
		ElementUtil.safeClick(searchAssetBox, driver);

		// 2. Type asset type
		ElementUtil.sendText(searchAssetBox, type, driver);
		...
		// 4. Select first asset from filtered list
		ElementUtil.safeClickFirstAsset(firstAssetSelect, driver);

		// 5. ❗ Clear search WITHOUT page reload
		WebElement searchBox = driver.findElement(searchAssetBox);
	...
	public void publishPlaybook() {
		...
		WaitUtil.waitForPageToLoad(driver, 60);
		WaitUtil.waitForElementVisible(driver, arrowClickPlaybook, 60);
		ElementUtil.clickWithRetry(arrowClickPlaybook, driver, 3); // Use robust click
		// WaitUtil.waitAndClick(driver, arrowClickPlaybook, 70);
